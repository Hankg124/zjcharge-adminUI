<template>
  <div class="index monitor-bg">
    <div v-show="sidebarVisible" class="index_l whitebg">
      <div class="qy_R">
        ...
        <div class="inedx_r_top_b">
          <StationTree :current-node-key="$route.query.id" :enable-station-only="true" @select-node="handleSelectNode"></StationTree>
        </div>
      </div>
    </div>
    ...
    <div class="index_r">
      <div class="tables">
        <div v-if="node.type == 'station'" style="text-align: left">
          ...
          >
            ...
            <div class="status">
              <div v-if="gunList.connectorStatus == 0" class="status-text">离线</div>
              <div v-if="gunList.connectorStatus == 1" class="status-text">空闲</div>
              <div v-if="gunList.connectorStatus == 2" class="status-text">已插枪</div>
              <div v-if="gunList.connectorStatus == 3" class="status-text">
                充电中
                <span>{{ (gunList.soc * 100).toFixed(0) }}%</span>
              </div>
              <div v-if="gunList.connectorStatus == 4" class="status-text">已锁定</div>
              <div v-if="gunList.connectorStatus == 255" class="status-text">故障</div>
              <div v-if="gunList.connectorStatus == 3" class="bar" :style="{ width: gunList.soc * 100 + '%' }"></div>
              <el-dropdown trigger="click" class="gun-more" @command="gunMoreClick">
                <span>
    ...
    <div v-if="!node.id" class="cc">请从左侧选择站点</div>
  </div>
</template>
<script>
...
import { repushGunStatus, sendQCode } from '@/api/basecharge/station';
...
.monitor-bg {
  background-image: radial-gradient(ellipse at 50% 50%, #1769a6 0%, #275794 52%, #195e81 90%);
