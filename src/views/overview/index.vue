<template>
  <div class="index">
    <div v-show="sidebarVisible" class="index_l whitebg">
      <div class="qy_R">
        ...
        <div class="inedx_r_top_b">
          <StationTree :enable-station-only="true" @select-node="handleSelectNode"></StationTree>
        </div>
      </div>
    </div>
    <div v-show="!sidebarVisible" class="left-float">
      <div @click="toggleSidebar">
        <i class="el-icon-d-arrow-right"></i>
      </div>
    </div>
    <div v-if="node.name" v-loading="loading" class="index_r whitebg">
      <div class="tables">
      ...
      <div class="tables mt10">
        <div class="pb20 bb">
          <el-radio-group v-model="timeType" class="mr10" @change="timeTypeCheck">
          ...
          <el-popover placement="top-start" title="客户运营商" trigger="hover">
            ...
            <template #reference>
              <i class="el-icon-question ml5" style="color: #abaeb4"></i>
            </template>
          </el-popover>
          <el-button circle class="el-icon-refresh ml5" @click="erchatInfo"></el-button>
        </div>
        <el-row v-if="incomeInfo" type="flex" class="ac count-label mt20" justify="space-between">
        ...
        <el-row v-if="incomeInfo" type="flex" class="ac count-number pb20 bb" justify="space-between">
          <el-col :span="3">
            <count-to
            ...
            />
            <span v-else>-</span>
            <span v-if="Number(incomeInfo.totalPower)" class="unit">度</span>
          </el-col>
          <el-col :span="3">
            <count-to
            ...
            <span v-if="incomeInfo.totalMoney" class="unit">元</span>
          </el-col>
          <el-col :span="3">
            <count-to
            ...
            <span v-if="incomeInfo.totalEMoney" class="unit">元</span>
          </el-col>
          <el-col :span="3">
            <count-to
            ...
            <span v-if="incomeInfo.totalSMoney" class="unit">元</span>
          </el-col>
          <el-col :span="3">
            ...
            <span v-if="incomeInfo.orderCount" class="unit">单</span>
          </el-col>
          <el-col :span="3">
            <count-to
              v-if="incomeInfo.totalMoney"
          ...
          <el-col :span="3">
            ...
            <span v-if="incomeInfo.totalDurant" class="unit">分钟</span>
          </el-col>
        </el-row>
        ...
        <div class="mt20 bb">
          ...
          <div class="title_name">
            ...
            <el-radio-group v-model="statsType" class="ml10" @change="statsTypeCheck">
    ...
    <div v-if="!node.name" class="cc">请从左侧选择站点</div>
  </div>
</template>
<script>
import { stationStats } from '@/api/basecharge/station';
import { getStationPrice } from '@/api/basecharge/priceTemplate';
import CountTo from '@/components/vue-count-to/index.js';
...
import { resetForm, parseTime } from '@/utils/common';
...
export default {
  ...
  methods: {
    ...
    handlePriceInfo(priceInfo) {
      ...
      this.priceTypes = [];
      if (priceInfo == null || priceInfo.priceList == null || priceInfo.priceList.length == 0) {
        return;
      }
      ...
      if (this.query.startTm) {
        ...
        stationStats(this.query).then((res) => {
          if (res.code == 200) {
            ...
            let xArray =
              this.interval == 'Hour'
                ? intervalArray.map((i, index) => `${index}:00`)
                : intervalArray.map((i, index) => (this.interval == 'Day' ? `${parseTime(i.tm, '{m}-{d}')}` : `${parseTime(i.tm, '{y}-{m}')}`));
            // 电费
            ...
            // 客单分段
            let amountArray = res.data.chargeMoneyPieList.map((i) => {
              return parseInt(i.value) ? { value: i.value, name: i.unit } : null;
            });
