<template>
  <div class="app-container">
    <el-form v-show="showSearch" ref="queryForm" :model="queryParams" :inline="true" :rules="rules">
      <el-row>
        ...
        <el-form-item v-if="queryParams.startChargeSeqStat == 4" label="占位状态" prop="occupyState">
        ...
        <el-form-item label="所属代理商" prop="agentId">
          <SelectAgent v-model="queryParams.agentId" :show-item-all="true" :clearable="true" />
        </el-form-item>
      </el-row>
      <el-row>
    ...
    <el-table v-loading="loading" :data="orderList" class="l-panel" border height="calc(100vh - 270px)" @selection-change="doSelectionChange">
      ...
      <el-table-column prop="totalMoney" label="金额" width="80" align="right">
        <template #default="scope">{{ (scope.row.totalPrice + scope.row.seatPayPrice) | unitPrice }}</template>
      </el-table-column>
      <el-table-column prop="couponMoney" label="优惠金额" width="80" align="right">
        <template #default="scope">
          <span v-if="scope.row.couponMoney > 0" style="color: #ff9920">- {{ scope.row.couponMoney | unitPrice }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="payPrice" label="实付金额" width="80" align="right">
        <template #default="scope">{{ (scope.row.payPrice + scope.row.seatPayPrice) | unitPrice }}</template>
      </el-table-column>
    ...
    <!-- 订单详情对话框 -->
    <el-drawer v-model:visible="open" :title="title" size="400px">
      <div class="el-drawer__content" style="calc(100vh - 40px)">
        ...
        <el-button v-if="form.startChargeSeqStat == 2" type="danger" size="mini" icon="el-icon-warning-outline" @click="handleStopCharge"
          >结束充电</el-button
        >
        <el-button v-if="form.startChargeSeqStat == 8" type="danger" size="mini" icon="el-icon-warning-outline" @click="showForceStopOrder"
          >处理异常</el-button
        >
        <!-- 处理异常对话框 -->
