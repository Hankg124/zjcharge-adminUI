<template>
  <div style="display: inline">
    ...
    <el-form-item prop="value">
      <el-select :disabled="detail" @change="setCityList" v-model="selectPro" size="large" style="width: 100px" placeholder="请选择省">
        <el-option v-for="(item, key) in proList" :key="key" :label="item.name" :value="item.id"></el-option>
      </el-select>
    </el-form-item>
    <el-form-item>
      <el-select :disabled="detail" @change="setAreaList" v-model="selectCity" size="large" style="width: 100px" placeholder="请选择市">
        <el-option v-for="(item, index) in cityList" :key="index" :label="item.name" :value="item.id"></el-option>
      </el-select>
    </el-form-item>
    <el-form-item>
      <el-select :disabled="detail" v-model="selectArea" @change="setData" size="large" style="width: 100px" placeholder="请选择区（县）">
    ...
    initData() {
      if (this.defaultValue) {
        ...
        if (this.defaultValue.substring(0, 6)) {
          ...
          console.log('666', this.selectArea);
        }
